...
>
> The sensor includes CEL `exprs` (combined with `exprLogicalOperator: or`) that (1) ignore pushes whose head commit message contains `[workflow]` and (2) only trigger when the head commit modifies files under `apps/my-service/app/`. Each expression binds explicit fields (`headMessage`, `headModified`) to the payload so CEL can evaluate the conditions. Update the expressions in `argo-events/sensor.yaml` (plus the workflow commit message) if you change these conventions.
