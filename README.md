...
> Keep the Git automation email at `workflow@example.com` (or adjust both the sensor filter and the workflow script defaults) so the system can ignore commits generated by the workflow itself.
...
>
> The sensor includes an `exprLogical` filter that ignores pushes whose head commit author email equals `workflow@example.com`. This prevents the workflow’s own `update-values` commit from re-triggering itself forever. If you change the bot identity (for example, by setting `GIT_EMAIL` in the `github-token` secret), update the expression in `argo-events/sensor.yaml` to match the email you use for automation.
...
- **Workflow keeps recreating**: if the sensor triggers on every push (including the workflow’s own `update-values` commit), ensure `argo-events/sensor.yaml` has an `exprLogical` filter that ignores the automation email (default `workflow@example.com`).
